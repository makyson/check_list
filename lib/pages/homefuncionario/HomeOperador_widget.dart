...
class HomeFuncionarioWidget extends StatefulWidget {
  const HomeFuncionarioWidget({Key? key}) : super(key: key);

  @override
  _HomeOperadorWidgetState createState() => _HomeOperadorWidgetState();
}

class _HomeOperadorWidgetState extends State<HomeFuncionarioWidget> {
  late HomeFuncionarioModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => HomeFuncionarioModel());
  ...
  Future<void> buscaroperadores() async {
    final response =
        await http.get(Uri.parse('${apilogin()}/checklist/mecanicos_v2'));
      ...
      setState(() {
        ...
        operador = data
            .map((item) => {
                  ...
                  'nome': item['nome'],
                  'funcao': item['funcao'],
                  'quantidadeDefeitosInicio': item['quantidadeDefeitosInicio'],
                })
            .toList();
  ...
  List<Map<String, dynamic>> filterOperador() {
    ...
    if (searchTerm.isNotEmpty) {
      ...
      filteredEquipamentos = filteredEquipamentos.where((equip) {
        ...
        bool nomeMatch = searchWords.any(
            (word) => StringSimilarity.compareTwoStrings(word, nome) > 0.2);
        bool funcaoMatch = searchWords.any(
            (word) => StringSimilarity.compareTwoStrings(word, funcao) > 0.35);
      ...
      filteredEquipamentos.sort((a, b) {
        double maxSimilarityA = searchWords
            .map((word) => StringSimilarity.compareTwoStrings(
                word, a['nome'].toLowerCase()))
            .reduce((a, b) => a > b ? a : b);
        double maxSimilarityB = searchWords
            .map((word) => StringSimilarity.compareTwoStrings(
                word, b['nome'].toLowerCase()))
            .reduce((a, b) => a > b ? a : b);
  ...
  void exibirSobreposicao(
      BuildContext context, String? imagem, IconData selicon) {
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  //mostrar a quantidadeDefeitosInicio em badge
                  if (equip['quantidadeDefeitosInicio'] != null &&
                      equip['quantidadeDefeitosInicio'] > 0)
                    Container(
                      ...
                      child: Text(
                        // if equip['quantidadeDefeitosInicio'].toString() maior que 999 entao 999 +,
                        equip['quantidadeDefeitosInicio'] > 999
                            ? '999+'
                            : equip['quantidadeDefeitosInicio'].toString(),
                              ...
                              children: [
                                ...
                                ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return buildOperadores(
                                        filterOperador()[index]);
                                  },
                                ),
                              ],
                            ),
